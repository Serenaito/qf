...
LAppSprite::LAppSprite(float x, float y, float width, float height, GLuint textureId, GLuint programId)
    : _rect()
{
    //qDebug("LAppSprite::LAppSprite:l:%d,r:%d,u:%d,d:%d",x.left,_rect.right,_rect.up,_rect.down);
    _rect.left = (x - width * 0.5f);
    _rect.right = (x + width * 0.5f);
    _rect.up = (y + height * 0.5f);
    _rect.down = (y - height * 0.5f);
    _textureId = textureId;
    //qDebug("LAppSprite::LAppSprite:l:%f,r:%f,u:%f,d:%f",_rect.left,_rect.right,_rect.up,_rect.down);
    // 何番目のattribute変数か
...
void LAppSprite::Render() const
{
    ...
    int maxWidth = LAppDelegate::GetInstance()->GetWindow()->width();
    int maxHeight = LAppDelegate::GetInstance()->GetWindow()->height();
...
bool LAppSprite::IsHit(float pointX, float pointY) const
{
    ...
    int maxWidth = LAppDelegate::GetInstance()->GetWindow()->width();
    int maxHeight = LAppDelegate::GetInstance()->GetWindow()->height();
    //qDebug("LAppSprite::IsHit:w:%f h:%f",pointX,pointY);
    //qDebug("LAppSprite::LAppSprite:l:%f,r:%f,u:%f,d:%f",_rect.left,_rect.right,_rect.up,_rect.down);
    if (maxWidth == 0 || maxHeight == 0)
    {
